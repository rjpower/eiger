...
public class StorageProxy implements StorageProxyMBean {
    ...
    private static Collection<InetAddress> getWriteEndpoints(String table,
        ...
        return ss.getTokenMetadata().getWriteEndpoints(tk, table,
                naturalEndpoints);
    }
     ...
     * other replicas
     */
    public static void sendMessages(String localDataCenter,
            ...
            for (Map.Entry<Message, Collection<InetAddress>> messages : entry
                ...
                while (iter.hasNext()) {
                    ...
                    CompactEndpointSerializationHelper.serialize(destination,
                            dos);
                    String id = MessagingService.instance().addCallback(
                            handler, message, destination);
                    dos.writeUTF(id);
                    if (logger.isDebugEnabled())
                ...
                String id = MessagingService.instance().sendRR(message, target,
                        handler);
                if (logger.isDebugEnabled())
    ...
    private static Runnable counterWriteTask(final IMutation mutation,
        ...
        return new DroppableRunnable(StorageService.Verb.MUTATION) {
            ...
            public void runMayThrow() throws IOException {
                ...
                logger.debug("counterWriteTask: " + cm + " to " + targets
                        + " sROW: " + cm.shouldReplicateOnWrite());
    ...
    private static List<Row> fetchRows(List<ReadCommand> initialCommands, ConsistencyLevel consistency_level) throws IOException, UnavailableException, TimeoutException
    {
        ...
        do
        {
            ...
            for (int i = 0; i < commands.size(); i++)
            {
                ...
                if ((dataPoint.equals(FBUtilities.getBroadcastAddress()) || dataPoint.equals(FBUtilities.getLocalAddress()))  && OPTIMIZE_LOCAL_REQUESTS)
                {
                ...
                {
                    assert false : "Proxying reads is not currently allowed in COPS2 " + dataPoint + " != " + FBUtilities.getBroadcastAddress() + " or " + OPTIMIZE_LOCAL_REQUESTS;
        ...
        try {
            ...
            for (AbstractBounds<RowPosition> range : ranges) {
                ...
                if (Table.open(command.keyspace).getReplicationStrategy() instanceof NetworkTopologyStrategy) {
                    String localDataCenter = DatabaseDescriptor
                            .getEndpointSnitch().getDatacenter(
                                    FBUtilities.getBroadcastAddress());
                    dcReplicationFactor = ((NetworkTopologyStrategy) Table
                            .open(command.keyspace).getReplicationStrategy())
                            .getReplicationFactor(localDataCenter);
                }
                if ((consistency_level == ConsistencyLevel.ONE
                        || overallReplicationFactor == 1 || dcReplicationFactor == 1)
                        && !liveEndpoints.isEmpty()
                        && liveEndpoints.contains(FBUtilities
                                .getBroadcastAddress())) {
                    if (logger.isDebugEnabled())
                        logger.debug("local range slice");

                    try {
                        rows.addAll(RangeSliceVerbHandler
                                .executeLocally(nodeCmd));
                        for (Row row : rows)
                    ...
                    for (int i = 0; i < liveEndpoints.size(); i++) {
                        System.out.println("live endpoint: "
                                + liveEndpoints.get(i));
                    }
                    assert false : "Client should send requests directly to their local nodes in COPS2 "
                            + overallReplicationFactor
                            + ":"
                            + dcReplicationFactor
                            + ":"
                            + liveEndpoints.isEmpty()
                    ...
                    RangeSliceResponseResolver resolver = new RangeSliceResponseResolver(
                            nodeCmd.keyspace, liveEndpoints);
                    ReadCallback<Iterable<Row>> handler = getReadCallback(
                            resolver, nodeCmd, consistency_level, liveEndpoints);
                    handler.assureSufficientLiveNodes();
                    for (InetAddress endpoint : handler.endpoints) {
                        MessagingService.instance().sendRR(nodeCmd, endpoint,
                                handler);
                        if (logger.isDebugEnabled())
     ...
     */
    static <T extends RingPosition> List<AbstractBounds<T>> getRestrictedRanges(
            final AbstractBounds<T> queryRange) {
        ...
        Iterator<Token> ringIter = TokenMetadata.ringIterator(
                tokenMetadata.sortedTokens(), queryRange.left.getToken(), true);
        AbstractBounds<T> remainder = queryRange;
        while (ringIter.hasNext()) {
            Token token = ringIter.next();
            /*
             * remainder can be a range/bounds of token _or_ keys and we want to
             * split it with a token: - if remainder is tokens, then we'll just
             * split using the provided token. - if reaminer is keys, we want to
             * split using token.upperBoundKey. For instance, if remainder is
             * [DK(10, 'foo'), DK(20, 'bar')], and we have 3 nodes with tokens
             * 0, 15, 30. We want to split remainder to A=[DK(10, 'foo'), 15]
             * and B=(15, DK(20, 'bar')]. But since we can't mix tokens and keys
             * at the same time in a range, we uses 15.upperBoundKey() to have A
             * include all keys having 15 as token and B include none of those
             * (since that is what our node owns). asSplitValue() abstracts that
             * choice.
             */
            T splitValue = (T) token.asSplitValue(queryRange.left.getClass());
            if (remainder == null
                    || !(remainder.left.equals(splitValue) || remainder
                            .contains(splitValue)))
                // no more splits
                break;
            Pair<AbstractBounds<T>, AbstractBounds<T>> splits = remainder
                    .split(splitValue);
            if (splits.left != null)
    ...
    public static List<Row> scan(final String keyspace, String column_family,
        ...
        List<AbstractBounds<RowPosition>> ranges = getRestrictedRanges(new Bounds<RowPosition>(
                leftPos, p.getMinimumToken().minKeyBound()));
    ...
    /**
     * Will mark this operation as pending, and then send out a check for each
     * dependency
     * 
     * @param keyspace
     *            also known as a "table"
     * @param locatorKey
     *            key associated with the blocked operation
     * @param timestamp
     *            uniquely identifies the blocked operation
     * @param deps
     *            the dependencies we want to check
     * @param completableVerbHandler
     *            object with a compete callback to be executed after all dep
     *            checks return
     */
    public static void checkDependencies(String keyspace,
            ByteBuffer locatorKey, long timestamp, Set<Dependency> deps,
        ...
        DepCheckCallback depCheckCallback = new DepCheckCallback(deps,
                completable);
        for (Dependency dep : deps) {
            List<InetAddress> localEndpoints = StorageService.instance
                    .getLocalLiveNaturalEndpoints(keyspace, dep.getLocatorKey());
            assert localEndpoints.size() == 1 : "Assumed for now";
            InetAddress localEndpoint = localEndpoints.get(0);

            // WL TODO: Group dep_checks going to the same node into 1 message,
            // like TransactionProxy.sendTransactionMessages
            MessagingService.instance().sendRR(new DependencyCheck(dep),
                    localEndpoint, depCheckCallback);
        }
        // DepCheckCallback will gather the responses and then complete the
        // blocked action
    }
}
